...
cd ../llm-runtime && make up RUNTIME=ollama MODE=native

# Container (CPU on Mac, GPU auto-detected on Linux)
cd ../llm-runtime && make up RUNTIME=ollama
```
...
**Download a GGUF** into `../llm-runtime/llamacpp/models/` (e.g. from huggingface.co).

**Start runtime:**
```bash
# Mac native (Metal — recommended)
LLAMACPP_MODEL=llama-3.2-3b-instruct-q4_k_m.gguf \
  cd ../llm-runtime && make up RUNTIME=llamacpp MODE=native

# Container (CPU on Mac, GPU auto-detected on Linux)
LLAMACPP_MODEL=llama-3.2-3b-instruct-q4_k_m.gguf \
  cd ../llm-runtime && make up RUNTIME=llamacpp
```
...
**Start runtime:**
```bash
cd ../llm-runtime && make up RUNTIME=vllm
```

**config.yaml local model block** (one entry — the loaded model):
```yaml
  # ── Local models — active runtime: vLLM ─────────────────────────────────
  # model_name must match VLLM_SERVED_MODEL_NAME in llm-runtime/.env
  - model_name: llama3.2
    litellm_params:
      model: openai/llama3.2
